...
try:
    ...
    try:
        ...
        try:
            ...
            root = tk.Tk()
            root.withdraw()
            messagebox.showerror("ARARAT Viewer", f"Erro fatal ao importar dependências:\n{e2}")
            root.destroy()
        except Exception:
            pass
        sys.exit(1)

APP_DIRS = path_utils.ensure_dirs()
path_utils.init_logging()
logger = logging.getLogger("ararat.viewer")
...
class ViewerApp:
    ...
    def __init__(self, dicom_dir=None, data_root=None, series_hint="t2tsetra"):
        ...
        if path_utils.is_frozen() and not data_root and not dicom_dir:
            config = {}
        ...
        self.samples_root = config.get("samples_root") if config else None
        
        self.input_root = str(Path(data_root).resolve()) if data_root else (str(Path(dicom_dir).resolve()) if dicom_dir else None)
        self.dicom_root = self.input_root
        self.series_hint = series_hint

        try:
            gt_status = gt_labels.preload_labels(self.input_root)
            if gt_status.get("ok"):
                logger.info("gt_labels_preloaded source=%s stats=%s", gt_status.get("source"), gt_status.get("stats"))
            else:
                logger.warning("gt_labels_indisponivel source=%s error=%s", gt_status.get("source"), gt_status.get("error"))
        except Exception:
            logger.exception("gt_labels_preload_falhou")
        ...
        self.patient_select_mode = False # novo modo de navegacao
        ...
        self._slots_normal = {
            "main": self.slot_main,
            "bottom_left": self.slot_bl,
            "bottom_right": self.slot_br,
        }
        self._slots_focus = {
            "main": [0.27, 0.33, 0.53, 0.62],
            "bottom_left": [0.27, 0.05, 0.265, 0.25],
            "bottom_right": [0.535, 0.05, 0.265, 0.25],
        }
        self.plane_flip_x = {p: False for p in ["axial", "coronal", "sagittal"]}
        
        # dados da roi candidata (preview)
        ...
        self.logo_img = None
        self.runtime_dirs = APP_DIRS
        self.app_root = str(path_utils.get_app_root())
        self.writable_root = str(path_utils.get_writable_root())
        logger.info("app_root=%s writable_root=%s", self.app_root, self.writable_root)
        assets_dir = path_utils.resolve_path("viewer", "assets")
        logo_path = assets_dir / "ararat_logo.png"
        self.ico_path = str(assets_dir / "ararat_logo.ico")
        if not logo_path.exists():
            png_candidates = list(assets_dir.glob("*.png"))
            if png_candidates:
                logo_path = png_candidates[0]
        
        if logo_path.exists():
            try:
                self.logo_img = mpimg.imread(str(logo_path))
                ...
                logger.exception("falha_logo_icone")
        else:
            print(f"[WARNING] Logo nao encontrada em {logo_path}")
            logger.warning("logo_nao_encontrada=%s", str(logo_path))

        self.export_dir = str(path_utils.resolve_writable_path("exports"))
        ...
        model_path = path_utils.resolve_path("inference", "models", "v1_prostatex", "model.joblib")
        if not model_path.exists():
            msg = f"Modelo não encontrado: {model_path}"
            logger.error(msg)
            path_utils.show_error_popup(f"{msg}\n\nErro - ver logs.")
    ...
    def discover_workspace(self):
        ...
        if not self.input_root or not os.path.exists(self.input_root):
            self.cases_list = []
            self.last_message = "Dataset não encontrado. Selecione a pasta de cases."
            logger.warning("dataset_nao_encontrado input_root=%s", self.input_root)
            return

        print(f"Analisando workspace: {self.input_root}")

        base_name = os.path.basename(self.input_root).strip()
        if base_name.isdigit() or re.match(r"^prostatex[-_\s]*\d{1,4}$", base_name, flags=re.IGNORECASE):
            if dicom_io.list_case_series(self.input_root):
                print("[INFO] Modo SINGLE CASE detectado (pasta de paciente).")
                self.is_samples_mode = False
        ...
        if os.path.exists(samples_dir) and os.path.isdir(samples_dir):
            ...
            if has_valid_subdirs:
                ...
                self.input_root = os.path.dirname(self.samples_root)
                self.is_samples_mode = True
            else:
                # caso c:
                if dicom_io.list_case_series(self.input_root):
    ...
    def _load_gt_for_case(self):
        ...
        try:
            ...
            self.gt_lesions = []
        if patient_id and not self.gt_lesions:
            self.last_message = f"GT indisponivel: sem labels para {patient_id}"
        try:
            st = gt_labels.get_labels_status()
        except Exception:
            st = {}
        self.gt_label_source = (st.get("source") if isinstance(st, dict) else None) or (self.gt_lesions[0].get("source") if self.gt_lesions else None)
        self.gt_labels_stats = st.get("stats") if isinstance(st, dict) else None
        self.gt_labels_error = st.get("error") if isinstance(st, dict) else None
        logger.info("gt_case_loaded case=%s patient_id=%s lesions=%s source=%s", case_name, patient_id, len(self.gt_lesions), self.gt_label_source)
    ...
    def validate_rois_for_current_series(self):
        ...
        for roi in self.rois:
            ...
            # converte centro do mundo para voxel na serie atual
    ...
    def load_current_series(self, center_mm=None):
        ...
        if cache_key in self._series_cache:
            ...
            try:
                ...
                logger.exception("erro_carregar_serie")
                traceback.print_exc()
                if self.fig: self.update_plot()
                return
    ...
    def run(self):
        # desativa hotkeys do matplotlib
        ...
        if not self.input_root or not os.path.exists(self.input_root):
            self.last_message = "Pressione O para abrir a pasta do paciente."
    ...
    def _draw_preview_fast(self):
        ...
        # determina posicao e parametros
    ...
    def _get_help_text(self):
        ...
        lines = [
            ...
            fmt_line("Double-click", "maximizar/restaurar painel"),
            ...
            fmt_line("L", "ir para voxel i,j,k"),
            "",
            "ROI (LESAO)",
    ...
    def _render_mpr_view(self, ax, plane):
        ...
        flip_x = False
        try:
            direction = meta_d.get("direction")
        except Exception:
            direction = None
        if plane in ["axial", "coronal"] and direction and isinstance(direction, (list, tuple)) and len(direction) == 9:
            try:
                d = np.array(direction, dtype=float).reshape(3, 3)
                u_vec = d[:, 0]
                idx = int(np.argmax(np.abs(u_vec)))
                sign = float(np.sign(u_vec[idx]))
                if idx == 0:
                    pos = "L" if sign > 0 else "R"
                    neg = "R" if sign > 0 else "L"
                else:
                    pos = ""
                    neg = ""
                if "R" in (neg, pos):
                ...
                flip_x = False
        self.plane_flip_x[plane] = bool(flip_x)
        state = self.view_state.get(plane)
        ...
        if xlim is not None and ylim is not None:
            if self.plane_flip_x.get(plane):
    ...
    def _apply_slots(self):
        slots = self._slots_focus if getattr(self, "layout_mode", "normal") == "focus" else self._slots_normal
        self.slot_main = slots["main"]
        self.slot_bl = slots["bottom_left"]
        self.slot_br = slots["bottom_right"]
        if self.ax_axial:
    ...
    def _toggle_maximize_panel(self, plane):
        if plane not in ["axial", "coronal", "sagittal"]:
            return
        if getattr(self, "layout_mode", "normal") != "focus":
            self.layout_mode = "focus"
            self.main_view = plane
        else:
            if (self.main_view or "axial") == plane:
                self.layout_mode = "normal"
            else:
                self.main_view = plane
        self._apply_slots()
    ...
    def _draw_orientation_labels(self, ax, plane):
        ...
        def axis_letter(vec):
            ...
            if idx == 0:
                ...
                pos = "P" if sign > 0 else "A"
                neg = "A" if sign > 0 else "P"
                return (neg, pos)
            else:
                pos = "S" if sign > 0 else "I"
                neg = "I" if sign > 0 else "S"
                return (neg, pos)
        if direction and isinstance(direction, (list, tuple)) and len(direction) == 9:
            ...
            if plane == "axial":
                ...
                return
            left_label, right_label = axis_letter(u_vec)
            top_label, bottom_label = axis_letter(v_vec)
        else:
            use_fallback = True
            if plane == "axial":
                ...
                return
        if plane in ["axial", "coronal"] and self.plane_flip_x.get(plane):
    ...
    def _draw_crosshair(self, ax, plane):
        ...
        if plane == "axial":
            ...
            return
        ax.axvline(x, color="yellow", linestyle="--", linewidth=0.8, alpha=0.45)
        ax.axhline(y, color="yellow", linestyle="--", linewidth=0.8, alpha=0.45)
    ...
    def update_plot(self):
        ...
        elif self.mode == "VOXEL_JUMP":
            mode_str = f"GO TO VOXEL i,j,k: {self.voxel_input_str}_ (Enter confirm, Esc cancel)"
        else:
        ...
        if self.show_gt:
            ...
            if getattr(self, "gt_labels_stats", None):
                info_panel_text += f"labels_stats={self.gt_labels_stats}\n"
            if not self.gt_lesions:
                if self.gt_patient_id and self.gt_label_source:
                    info_panel_text += f"GT indisponivel: sem labels para {self.gt_patient_id}\n"
                elif getattr(self, "gt_labels_error", None):
                    info_panel_text += f"GT indisponivel: {self.gt_labels_error}\n"
                else:
                    info_panel_text += "GT indisponivel: labels nao carregadas ou mapping ausente\n"
    ...
    def _toggle_gt(self):
        try:
            ...
            gt_status = {"ok": False, "source": None, "error": "falha_ao_carregar_labels"}

        if not self.show_gt:
            self.show_gt = True
            if self.cases_list and self.current_case_idx >= 0:
                self._load_gt_for_case()
            if not self.gt_lesions:
                if not gt_status.get("ok"):
                    self.last_message = "GT indisponivel: labels nao carregadas"
                    try:
                        path_utils.show_error_popup(
                            f"GT indisponível.\n\nMotivo: {gt_status.get('error')}\n\n"
                            "O EXE inclui um pacote interno de labels, mas ele não pôde ser carregado.\n"
                            "Veja o log em logs/ararat_viewer.log.",
                            title="ARARAT Viewer - GT",
                        )
                    except Exception:
                        pass
                else:
                    if not self.cases_list or self.current_case_idx < 0:
                        self.last_message = "Abra um paciente para usar GT"
                    else:
                        self.last_message = "GT indisponivel para este caso"
            else:
                if self.meta is not None and self.np_vol is not None:
                    try:
                        self._jump_to_gt_slice()
                    except Exception:
                        pass
                if self.gt_patient_id:
                    self.last_message = f"GT ON: {self.gt_patient_id}"
                else:
                    self.last_message = "GT ON"
        else:
            self.show_gt = False
            self.last_message = "GT OFF"
        self.update_plot()
    ...
    def on_click(self, event):
        ...
        if event.dblclick and event.button == 1:
            self._toggle_maximize_panel(plane)
            return
        if event.button == 2:
    ...
    def on_key(self, event):
        try:
            ...
            if self.mode == "NORMAL" and event.key == "escape" and getattr(self, "layout_mode", "normal") == "focus":
                self.layout_mode = "normal"
                self._apply_slots()
                self.update_plot()
                return
            ...
            if self.mode == "NORMAL" and event.key == "l":
                self.mode = "VOXEL_JUMP"
                self.voxel_input_str = ""
                self.last_message = "Digite i,j,k e pressione Enter"
                self.update_plot()
                return
            ...
            if self.mode == "VOXEL_JUMP":
                if event.key == "enter":
                    vals = re.findall(r"-?\d+", self.voxel_input_str or "")
                    if len(vals) >= 3:
                        try:
                            i, j, k = (int(vals[0]), int(vals[1]), int(vals[2]))
                            self._set_center_voxel(i, j, k)
                            self.mode = "NORMAL"
                            self.voxel_input_str = ""
                            self.last_message = f"Ir para voxel ({i},{j},{k})"
                        except Exception:
                            self.last_message = "Entrada invalida (use i,j,k)"
                    else:
                        self.last_message = "Entrada invalida (use i,j,k)"
                    self.update_plot()
                elif event.key == "escape":
            ...
            # MODO NORMAL
                ...
                # atalhos 1-9 (apenas se nao for 0)
                idx = int(event.key) - 1
    ...
    def _export_roi_assets(self, roi):
        ...
        try:
            ...
            # calculo de elipse para o export (mesma logica do _draw_roi_sphere)
            p_center = dicom_io.voxel_to_mm(ci, cj, ck, self.meta)
            p_here = dicom_io.voxel_to_mm(ci, cj, self.current_slice, self.meta)
            dz_mm = abs(p_here[2] - p_center[2])
    ...
    def open_last_export_dir(self):
        ...
        try:
            ...
            logger.exception("erro_abrir_pasta_export")
        self.update_plot()
    ...
    def open_data_root(self):
        ...
        try:
            ...
            root = tk.Tk()
            root.withdraw()
            new_root = filedialog.askdirectory(title="Selecione a pasta do PACIENTE (caso) com DICOM")
            root.destroy()
            
            if new_root:
                old_root = self.input_root
                chosen = str(Path(new_root).resolve())
                self.input_root = chosen
                self.samples_root = None
                self.discover_workspace()

                if self.is_samples_mode and self.samples_root and os.path.isdir(self.samples_root):
                    try:
                        root = tk.Tk()
                        root.withdraw()
                        case_dir = filedialog.askdirectory(title="Selecione a pasta do PACIENTE dentro de SAMPLES", initialdir=self.samples_root)
                        root.destroy()
                    except Exception:
                        case_dir = None
                    if not case_dir:
                        ...
                        self.samples_root = None
                        self.discover_workspace()
                        self.last_message = "Selecao cancelada. Pressione O para escolher um paciente."
                        if self.fig:
                            self.update_plot()
                        return
                    self.input_root = str(Path(case_dir).resolve())
                    self.samples_root = None
                    self.discover_workspace()
                ...
                if self.cases_list:
                    self._save_config()
                    self.load_case(0)
                    self.last_message = f"Paciente carregado: {self.cases_list[self.current_case_idx]}"
                else:
                    self.input_root = old_root
                    self.samples_root = None
                    self.discover_workspace()
            ...
            logger.exception("erro_abrir_seletor")
    ...
    def export_all_to_pipeline(self):
        ...
        try:
            ...
            logger.exception("erro_export_pipeline")
            traceback.print_exc()
...
def main():
    ...
    try:
        ...
        logger.exception("falha_fatal_viewer")
        path_utils.show_error_popup(f"Erro - ver logs.\n\n{e}")
        raise
